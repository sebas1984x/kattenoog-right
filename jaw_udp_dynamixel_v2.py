...
import snap7
from snap7.util import get_bool  # only for bit extraction convenience
...
def deg_to_tick(deg: float) -> int:
    # Map 0..360 naar 0..4095, daarna clamp elders op HW-limits
...
def main():
    ...
    # Dummy read to ensure connectivity
    try:
        _ = plc.db_read(args.db_cmd, 0, 12)  # minimaal 12B conform layout
    except Exception as e:
    ...
    while True:
        ...
        # 1) Lees Command DB
        try:
            # 0..9 minimaal, maar we lezen 12B voor marge
            cmd_raw = plc.db_read(args.db_cmd, 0, 12)
        except Exception as e:
        ...
        # 3) Als nieuw commando: bereken en schrijf profielen + doelpositie
        ...
        if rcP != COMM_SUCCESS:
            pos_fb_ticks = 0
        # Present Velocity (signed)
        vel_fb_raw, rcV, erV = pk.read4ByteTxRx(ph, args.id, ADDR_PRESENT_VELOCITY)
        if rcV != COMM_SUCCESS:
            vel_fb_raw = 0
        # Dynamixel velocity is in vendor LSB; we geven het als 0..1000 schaal terug
        # Zonder absolute rpm-schaal: normaliseer tov vel_lim
        vel_fb_abs = abs(vel_fb_raw)
        vel_fb_scaled = int(clamp(round((vel_fb_abs / max(1, vel_lim)) * 1000.0), 0, 1000))

        # Positie feedback terug op dezelfde 0..1000 schaal:
        # map ticks -> degrees -> schaal 0..1000 binnen SW-range
        pos_fb_deg = tick_to_deg(pos_fb_ticks)
        # projecteer op SW window
        if sw_max > sw_min:
            pos_fb_u = (clamp(pos_fb_deg, sw_min, sw_max) - sw_min) / (sw_max - sw_min)
        else:
            pos_fb_u = 0.0
        if get_bool(cmd_raw, 6, 0):  # jaw_invert
        ...
        # 6) Wachttijd om op target loopfrequentie uit te komen
